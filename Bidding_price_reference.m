...
%原始条件
Cpri=[];
Cpri=[Cpri,0.1<=pi_DA<=1.3];%报价范围约束
Cpri=[Cpri,Smin<=S<=Smax,S(:,1)==0.25*0.95*Pch(:,1)-0.25*Pdis(:,1)/0.95+deltaS(:,1),
    S(:,2:96)==S(:,1:95)+0.25*0.95*Pch(:,2:96)-0.25*Pdis(:,2:96)/0.95+deltaS(:,2:96),
    0==S(:,96)+lastS];%电量约束
%目标函数
Obj=0.25*sum(sum((Price_DSO*ones(1,96)).*Pg))-sum(sum(Lagrant_balance.*PL))+sum(sum((Pg_step*ones(1,96)).*Lagrant_G_right))+sum(sum(Pf_limit*ones(1,96).*Lagrant_L_left))+sum(sum(Pf_limit*ones(1,96).*Lagrant_L_right));
%约束条件
...
result.Pf=double(Pf);
pi_DA=double(pi_DA);
pi_DA_reference=round(100*pi_DA)/100;
save('Bid_reference','pi_DA_reference','result');%参考报价